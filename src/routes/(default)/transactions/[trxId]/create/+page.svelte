<script lang="ts">
	...
	import { addTransactionItemsAPI } from "$lib/api/transactionAPI";
	import type { addedItemsForm, typeProductForm } from "$lib/type/productType";
	import { ButtonGroup, Table, TableHead, TableHeadCell, TableBody, TableBodyRow, TableBodyCell, Input, Dropdown, Search, DropdownGroup, Spinner, Card, Badge, GradientButton, Modal, Button } from "flowbite-svelte";
...
<div class="flex flex-col h-[95%] gap-3 p-2">
  ...
  <div class="flex h-full flex-row bg-white/30 rounded-md shadow-[0_8px_30px_rgba(0,0,0,0.1)] backdrop-blur-[6.1px]">
    <form class="flex h-full flex-row p-2 gap-2" onsubmit={() => confirmPopup = true}>
        ...
        <Table striped class="w-full border-collapse bg-white/50 shadow-[0_8px_30px_rgba(0,0,0,0.1)] backdrop-blur-[6.1px] table-fixed" hoverable={true}>
          ...
          <TableBody>
            ...
            <TableBodyRow class=" bg-linear-to-tr from-white/30 to-white/20 bg-transparent! rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.1)] backdrop-blur-[6.1px] text-xs">
              ...
              <TableBodyCell>
                <Input type="number" max={item.initial_quantity} bind:value={item.product_quantity} required/>
              </TableBodyCell>
      ...
      <div class="flex flex-col w-2/6 max-w-2/6">
        <Card class="flex flex-col gap-2 p-2 bg-blue-600/50 backdrop-blur-3xl">
          ...
          <GradientButton type="submit" shadow color="blue" class={!loading? "cursor-pointer" : "cursor-loading opacity-50"} disabled={loading}>Create Order</GradientButton>
        </Card>
      </div>
      {#if confirmPopup}
